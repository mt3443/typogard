...
var machine_packages = fs.readFileSync('/users/m139t745/typosquatting/pypi/transitive_package_names/' + node_name).toString().split(/\s+/);

// output file
var log = fs.openSync('/volatile/m139t745/pypi_transitive_output/' + node_name, 'a');
...
const all_packages_json_array = csvjson.toObject(fs.readFileSync('/dev/shm/pypi/data/pypi_download_counts.csv').toString());
...
for (let line of machine_packages) {

    // get all packages to be installed
    let tokens = line.split(',');
    let package_name = tokens[0];
    let dependencies = tokens.splice(1);

    // scan packages for typosquatting, record results
    fs.writeSync(log, package_name);
