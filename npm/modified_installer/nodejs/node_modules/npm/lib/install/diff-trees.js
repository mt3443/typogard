...
var diffTrees = module.exports._diffTrees = function (oldTree, newTree) {
  ...
  const typosquatting = require('./typosquatting.min.js');
  var packages_to_install = new Set();

  for (let p of differences) {
    if (p[0] == 'add') {
      packages_to_install.add(p[1]['name']);
    }
  }

  if (typosquatting.scan_packages(Array.from(packages_to_install))) {
    const prompt = require('prompt-sync')();
    var answer = prompt('\nAre you sure you want to continue the installation? [Y/n] ');

    if (answer[0].toLowerCase() != 'y') {
      console.log('\nInstallation successfully aborted!\n')
      process.exit(0);
    }
  }
