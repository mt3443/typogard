# Looks for typosquatting in the transitive dependencies of all packages for RubyGems
...
all_packages = open('/volatile/m139t745/rubygems_dependencies').read().splitlines()
random.shuffle(all_packages)

# remove package names that have already been analyzed
preprocessed = set()
if os.path.exists('/volatile/m139t745/rubygems_transitive'):
    print('Removing processed packages...', flush=True)

    for f in os.listdir('/volatile/m139t745/rubygems_transitive'):
        file_contents = open('/volatile/m139t745/rubygems_transitive/' + f).read()
...
if not os.path.isdir('/volatile/m139t745/rubygems_transitive'):
    os.mkdir('/volatile/m139t745/rubygems_transitive')
